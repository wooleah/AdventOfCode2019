...
function getOutput(input) {
  ...
  while(currentOpcodePos < inputArr.length) {
    // analyze opcode
    opcodeInput = inputArr[currentOpcodePos].toString();
    opcodeProtocol = Number(opcodeInput.slice(-2));
    input1Mode = opcodeInput[opcodeInput.length - 3] ? paramMode.IMMEDIATE : paramMode.POSITION;
    input2Mode = opcodeInput[opcodeInput.length - 4] ? paramMode.IMMEDIATE : paramMode.POSITION;
    
    switch (opcodeProtocol) {
      case opcode.INPUT:
        outputPos = inputArr[currentOpcodePos + 1];
        inputArr[outputPos] = AIR_CONDITIONER_UNIT_ID;
        nextOpcodePos = 2;
        break;
      case opcode.OUTPUT:
        debugger;
        input1Pos = inputArr[currentOpcodePos + 1];
        output = inputArr[input1Pos];
        nextOpcodePos = 2;
        break;
      case opcode.ADD:
      case opcode.MULTIPLY:
        if (input1Mode === paramMode.POSITION) {
          input1Pos = inputArr[currentOpcodePos + 1];
          input1 = inputArr[input1Pos];
        } else if (input1Mode === paramMode.IMMEDIATE) {
          input1 = inputArr[currentOpcodePos + 1];
        }

        if (input2Mode === paramMode.POSITION) {
          input2Pos = inputArr[currentOpcodePos + 2];
          input2 = inputArr[input2Pos];
        } else if (input1Mode === paramMode.IMMEDIATE) {
          input2 = inputArr[currentOpcodePos + 2];
        }
    ...
    currentOpcodePos += nextOpcodePos;
  }
